    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if coreDataItems.count > 0 {
            do {
                ...
                let color = (UIColor(hexString: items.value(forKey: "color") as! String))
                ...
                cell.coloredCircle.tintColor = color
            } catch {
                print(error)
            }
        }
...
extension HomeViewController: DataScannerViewControllerDelegate {
    func dataScanner(_ dataScanner: DataScannerViewController, didTapOn item: RecognizedItem) {
        ...
        case .barcode(let barcode):
            if let str = barcode.payloadStringValue {
                searchController.searchBar.text = str
                searchWithBarcodeText(searchText: str)
